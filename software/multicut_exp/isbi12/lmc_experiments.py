...
def isbi12_lifted_multicut(ds_train_str, ds_test_str,
    ...
    if ds_test_str == "isbi2012_train":
        ...
        seg_name = "_".join( ["lmcresult", str(seg_id_test),
            str(lifted_feats_list), "seg"] ) + ".h5"
        seg_path = os.path.join(res_folder, seg_name)
        vigra.writeHDF5(mc_seg, seg_path, "data")

        # save binary edges
        edge_name = "_".join( ["lmcresult", str(seg_id_test),
            str(lifted_feats_list), "edges"] ) + ".tif"
...
if __name__ == '__main__':
    ...
    for seg_id in (0,1):
        ...
        ri, vi = isbi12_lifted_multicut("isbi2012_train", "isbi2012_train",
            seg_id, seg_id,
            local_feats_list, lifted_feats_list,
            mc_params)

        isbi12_lifted_multicut("isbi2012_train", "isbi2012_test",
            seg_id, seg_id,
            local_feats_list, lifted_feats_list,
            mc_params)
