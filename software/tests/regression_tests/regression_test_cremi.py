...
from multicut_src import ExperimentSettings, MetaSet
...
def init_regression_test_cremi(meta, cache_folder, top_folder):
    for sample in ('A','B','C'):
        for postfix in (0,1):
            sample_name = 'sample%s_%i' % (sample, postfix)
            data_folder = os.path.join(top_folder, sample_name)
            init_train(meta, cache_folder, data_folder, sample_name)
...
def regression_test_cremi(cache_folder, top_folder, with_lmc = True):
    ...
    if not os.path.exists( os.path.join(cache_folder, 'sampleA_0_train') ):
        init_regression_test_cremi(meta, cache_folder, top_folder)
    meta.load()
    ...
    for sample in ('A','B','C'):
        for postfix in (0,1):
            ...
            trainsets   = [meta.get_dataset(ds) for ds in train_names]
            mc_results[ds_test] = run_mc(
                    trainsets,
                    meta.get_dataset(ds_test),
                    feature_list,
                    params)
...
if __name__ == '__main__':
    regression_test_cremi(
            '/mnt/data/stuff/regression_tests_master',
